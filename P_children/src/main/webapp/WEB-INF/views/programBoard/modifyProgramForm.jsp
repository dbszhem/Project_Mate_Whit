			...
			<div class="wrap write_form">
				<div class="write_save">
					<table class="table table-striped">
						<form name="modifyProgramForm"
							action="${contextPath }/programBoard/modifySaveForProgram?num=<%=num2 %>"
							...
							<tr>
								...
								<td><input type="text" name="title"
									value="${programBoardDTO.title }"><br></td>
							</tr>
							<tr>
								<th><b> 견 종 </b><br></th>
								<td><input list="petKind_list" id="petKind" name="petKind"
									placeholder="견종을 검색해보세요" value="${programBoardDTO.petKind}">
									<datalist id="petKind_list">
										...
										<option value="오스트랄리안_테리어"
											label="오스트랄리안 테리어 (Australian Terrier)" />
										<option value="오스트레일리안_캐틀_독"
								...
								<th><b> 강아지 이름 </b></th>
								<td><input type="text" name="mateName"
									value="${programBoardDTO.mateName }" /></td>
							</tr>
							<tr>
								<th>강아지 사진</th>
								<td><input type="file" name="file" onchange="readURL(this)" />
									<c:if test="${programBoardDTO.mateImage == 'nan'}">
										<div class="noImageComment">
											<b>이미지가 없습니다...</b>
										</div>
										<img src="#" id="preview" width="100px" height="100px">
									</c:if> <c:if test="${programBoardDTO.mateImage != 'nan'}">
										<img
											src="${contextPath }/board/download?file_name=${programBoardDTO.mateImage}"
											width="200px" height="200px">
									</c:if></td>
								<th>위치</th>
								<td><input list="position_largeScale_list" id="position1"
									name="position1" onchange="position_1Selected()"
									value="${programBoardDTO.position1 }" placeholder="위치(시)">
								...
								<br> <input list="position_smallScale_list" id="position2"
									name="position2" value="${programBoardDTO.position2 }"
									placeholder="위치(구)"> <!-- 위치(시) 선택시 위치(구) 내용 나오는 위치 -->
									<div id="smallScaleDiv"></div></td>
							</tr>
							<tr>
								<th rowspan="3"><b> 내 용 </b></th>
								<td rowspan="3" style="right-padding: 0"><textarea
										name="content" rows="10" cols="50">${programBoardDTO.content }</textarea></td>
								<th style="left-padding: 0">시 작 날 짜 <br>
								<br> 시작 시간
								</th>
								<td><input type="text" id="datepicker1" name="startDate"
									value="${programBoardDTO.startDate}"> <br>
								<br> <input type="time" name="startTime" id="startTime"
									value="${programBoardDTO.startTime}"></td>
							</tr>
							<tr>
								<th style="left-padding: 0">종 료 날 짜 <br>
								<br> 종료 시간
								</th>
								<td><input type="text" id="datepicker2" name="endDate"
									value="${programBoardDTO.endDate}"><br>
								<br> <input type="time" name="endTime" id="endTime"
									value="${programBoardDTO.endTime}"></td>
							</tr>
							<tr>
								<th style="left-padding: 0">프 로 그 램 비 용</th>
								<td colspan="3"><input type="text" name="priceForProgram"
									id="priceForProgram"
									value="${programBoardDTO.priceForProgram }"
									placeholder="숫자만 입력하세요" /> <b>원</b></td>
								<td><b> 현황</b></td>
								<td><input list="state_list" type="text" id="state"
									name="state" value="${programBoardDTO.state }"> <datalist
										id="state_list">
										<option value="예약가능" label="예약 가능" />
										<!-- 초기값 -->
										<option value="승인대기" label="승인 대기" />
										<!-- 승인대기는 필요없을 것 같습니다..? -->
										<option value="결재완료" label="결재 완료" />
										<!-- 자신이 결재 했을 때 받는 것은 결재 완료 -->
										<option value="예약완료" label="예약 완료" />
										<!-- 참여 인원이 꽈 차면 예약 완료 -->
									</datalist></td>
							</tr>
							<input type="hidden" name="write_no"
								value="${programBoardDTO.write_no}">
						</form>
					</table>
